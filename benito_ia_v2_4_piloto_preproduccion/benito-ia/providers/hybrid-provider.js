window.BenitoHybridProvider={
  async generate(query,state){
    ...
    const usage=window.BenitoUsageLimiter?.canUse();
    if(usage&&!usage.allowed){
      return{meta:{mode:'limit'},messages:[{
        title:'Límite diario del piloto alcanzado',icon:'⏳',badge:'Uso responsable',
        response:'Para cuidar los recursos de esta versión piloto, se alcanzó el límite diario de consultas en este navegador.',
        actions:[{label:'Contacto institucional',href:'contacto.html'}]
      }]};
    }
    window.BenitoUsageLimiter?.increment();
    ...
    try{
      ...
      window.BenitoTelemetry?.add('query',{
        query,profile:state.profile||state.inferredProfile,provider:raw.provider,
        confidence:raw.confidence,success:true,
        latencyMs:Math.round(performance.now()-start),sourceCount:sources.length
      });
      return{
      ...
      window.BenitoTelemetry?.add('query',{query,profile:state.profile||state.inferredProfile,provider:'fallback-rag',success:false,latencyMs:Math.round(performance.now()-start)});
